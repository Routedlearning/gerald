...
- [x] **Removed `--history` CLI flag** — single write path through `storage.save()` only
...
| Path | Role | Notes |
|---|---|---|
| `/root/projects/fitness-coach/adaptation.py` | Rule engine | Missed sessions, RPE, 5k trends, weight stalls |
| `/root/projects/fitness-coach/cli.py` | CLI entry point | Updated: added `--feedback` and `--feedback-summary`; removed `--history` |
| `/root/projects/fitness-coach/feedback_parser.py` | Message parser | New; pure functions, platform-agnostic, no I/O |
| `/root/projects/fitness-coach/nutrition.py` | Macro targets | 2465 kcal / 220 g protein / 101 g fat / 167 g carbs |
| `/root/projects/fitness-coach/planner.py` | Day templates | Updated interval language: "400m hard effort / 400m walk or easy jog" |
| `/root/projects/fitness-coach/storage.py` | Persistence | Updated: schema v2, feedback_log validation, backward-compat migration |
| `/root/projects/fitness-coach/workout_history.json` | State file | Schema v2; feedback_log present; 0600 permissions |
| `/root/projects/fitness-coach/backups/` | Backup directory | Timestamped backups + quarantine files; 0600; max 30 |
| `/root/projects/fitness-coach/logs/backup.log` | Backup log | Manual backup script output |
| `/root/projects/fitness-coach/logs/cron_dispatch.log` | Cron log | Wrapper exit code + stdout/stderr |
| `/root/projects/fitness-coach/scripts/backup_workout_history.sh` | Backup helper | Manual backup trigger |
| `/root/projects/fitness-coach/scripts/restore_workout_history.sh` | Restore helper | `--list`, `--latest`, named, validation, 0600 |
| `/root/projects/fitness-coach/scripts/cron_dispatch.sh` | Cron wrapper | Executable; calls CLI; logs; fallback on failure |
| `/root/projects/fitness-coach/simulate_day.py` | Simulator | Replay day script for manual testing |
| `/root/projects/fitness-coach/ARCHITECTURE.md` | Architecture doc | Updated to v2; feedback logging design |
| `/root/projects/fitness-coach/PROJECT_STATUS.md` | Handoff | This file |
...
/root/projects/fitness-coach/scripts/restore_workout_history.sh /root/projects/fitness-coach/backups/<filename>.json

# Manual backup (outside of automatic saves)
bash /root/projects/fitness-coach/scripts/backup_workout_history.sh
```
...
| Feedback data in `feedback_log` (not yet fed into adaptation engine) | Info | `feedback_log` is ready; integration is future work |
...
```bash
cd /root/projects/fitness-coach && python3 cli.py --feedback "Done. RPE 7. Energy 4."
cd /root/projects/fitness-coach && python3 cli.py --feedback-summary
```
...
```bash
cd /root/projects/fitness-coach && python3 cli.py --week-start 2026-06-29
```
